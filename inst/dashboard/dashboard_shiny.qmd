...
get_applicant_list <- function(json_folder) {
...
observeEvent(input$loadFiles, {
    showModal(modalDialog(
      ...
      footer = tagList(
          actionButton("loadFiles", "Load local JSON files"),
          actionButton("loadDemo", "Demo data")
        )
    ))
  })
...
observeEvent(input$loadFiles, {
    ...
    # fileInput("loadFiles2", "Choose JSON files", multiple=TRUE, accept=".json")
    # file <- input$file1
    # ext <- tools::file_ext(file$datapath)
    # req(file)
    # validate(need(ext == "json", "Please select only json files."))
    # 
    # print(file)
    # print(ext)
    # content <- read_json(file, simplifyVector = TRUE)
    # print(content)
})
  
observeEvent(input$loadFiles, {
  print("loading demo files ...")
  DAT <- reactive({get_applicant_list(system.file("extdata/demo_profiles", package="RESQUER"))})
  print(str(DAT()))
})

system.file("extdata/demo_profiles", package="RESQUER")
```
...
### Row Co-author network / Internationality {height=25%}
::: {.card title="Co-author network / Internationality"}
```{r network_text}
#| context: server
output$network_text <- function(){applicant()$network$result_string}
```
```{r network_text_output}
