...
*(Note: The Overall Rigor Score reported here is only based on the four transparency categories, excluding ***Theorizing***. Therefore it can differ from the Overall Score in the individual report.)*
...
horizontal_bar_chart(c(1, 1, 1), colors = brewer.pal(3, "Set1"), 300, 80, labels=c(
"Students", 
"General population", 
...
horizontal_bar_chart(c(1, 1, 1, 1, 1), colors = brewer.pal(5, "Set2"), 300, 130, labels=c("Questionnaire", "Behavioral", "Physiological","Interviews / Content Data / Content Coding", "Meta Analysis")) |> HTML()
```
:::
::::
...
for (i in 1:nrow(tab_samples)) {
  ...
  Sample_Type <- factor(
    x = na.omit(applicant_list[[i]]$pubs$P_Sample),
  ...
  tab_samples$sample_type[i] <- horizontal_bar_chart(Sample_Type_count, colors = brewer.pal(3, "Set1"), 120, 40)
  
  
  # Types of data
  #------------------------------------------
  
  df <- applicant_list[[i]]$pubs |> select(contains("P_Data_Type_"))
  ...
  data_type_list <- c(
    "Questionnaire" = sum(df |> select(contains("Questionnaire")), na.rm=TRUE),
  ...
  tab_samples$data_type[i] <- horizontal_bar_chart(
    data_type_list, colors = brewer.pal(5, "Set2"), 120, 40)
}
