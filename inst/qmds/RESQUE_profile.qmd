...
params:
  ...
  json_path: "/Users/felix/LMU/DGPs Kommission Open Science/RESQUE/Workshop Allg1/resque_mückstein_1740406498057.json"
---
...
if (!"P_Sample" %in% names(applicant$pubs)) {applicant$pubs$P_Sample <- NA}
if (!"P_Sample_CrossCultural" %in% names(applicant$pubs)) {applicant$pubs$P_Sample_CrossCultural <- NA}
applicant$pubs$P_Sample[is.na(applicant$pubs$P_Sample)] <- FALSE
```


Type of population/sample and cultural background for the `r sum(applicant$pubs$P_Data_Source_NewOwn & applicant$pubs$P_Sample != "Other", na.rm=TRUE)` papers with own new data collection ("Other" excluded):
...
Sample_Type <- factor(
  x = na.omit(applicant$pubs$P_Sample),
...
if (any(!is.na(applicant$pubs$P_Sample_RareOther))) {
  res <- "**Rare samples used in studies:**\n\n"
  
  rare_sample_position <- which(!is.na(applicant$pubs$P_Sample_RareOther))
  for (i in rare_sample_position) {
    res <- paste0(res, "- ", applicant$pubs$P_Sample_RareOther[i], "\n")
    if (!is.na(applicant$pubs$P_SampleSize[i])) {
      res <- paste0(res, "  - <span style='font-size:80%;'>*Sample Size*: ", applicant$pubs$P_SampleSize[i], "</span>\n")
    } else {
...
|-|-|-|-|-
| `r sparkpie(applicant$OS_pie$OpenData)`  | `r sparkpie(applicant$OS_pie$Prereg, sliceColors=c("#EEEEEE", "red", "#00FF00", "#00FF00"))`| `r sparkpie(applicant$OS_pie$OpenMaterial)` | `r sparkpie(applicant$OS_pie$OpenCode)` | `r sparkpie(applicant$OS_pie$Repro)`  
...
pop_sel$merit <- cell_spec(
  ...
  popover = spec_popover(
    content = pop_sel$P_MeritStatement,
...
high_impact <- pop_sel[pop_sel$Popularity %in% c("Top 0.01%", "Top 0.1%", "Top 1%", "Top 10%"), ]
low_impact <- pop_sel[!pop_sel$Popularity %in% c("Top 0.01%", "Top 0.1%", "Top 1%", "Top 10%"), ]
```
...
if (nrow(high_impact) > 0) {
  ...
  kable(pop_sel[pop_sel$Popularity %in% c("Top 0.01%", "Top 0.1%", "Top 1%", "Top 10%"), ], escape = FALSE, table.attr = "class='table table-sm table-striped small' quarto-disable-processing=true") |> 
