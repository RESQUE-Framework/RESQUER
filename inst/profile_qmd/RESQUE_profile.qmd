...
format:
  html:
    toc: true
    toc-location: left
    css: assets/profile_styles.css
filters:
  - path: offcanvas
    at: pre-quarto
params:
  FullName: "Max Mustermann Demoprofile"
  json_path: '/Users/felix/Documents/Github/RESQUE-Framework/RESQUER/_test/report_testsuite/jsons/resque_schönbrodt_2026-03.json'
  show_inter: true
---
...
# RESQUER::render_profile('/Users/felix/LMU/DGPs Kommission Open Science/RESQUE/resque_schönbrodt_2026-03.json')
...
extra_meta <- extra_meta[!extra_meta %in% c("type", "position", "LastName", "FirstName", "YearPhD", "AcademicAgeBonus", "HonestyDisclaimer_Yes", "FullName", "AcademicAge", "OA_author_id", "date_exported", "date_added", "date_created", "date_modified", "date_analysis", "MeritStatement", "P_MeritStatement", "forms", "version")]
...
Other parts contain an evaluative aspect: For example, research that is reproducible, which allows independent auditing because it provides open data and scripts is, *ceteris paribus*, better than research that does not have these aspects. Research outputs with these objective quality criteria of methodological rigor can gain "bonus points" which are summed across all provided research outputs and contribute to the [Rigor Profile Overview](#indicators_of_research_transparency_and_reproducibility).

::: {.column-margin}
<a class="methods-pill" href="#rigor-bias-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="rigor-bias-offcanvas"><i class="bi bi-journal-text"></i>Methods: Rigor score bias</a>
:::

:::: {#rigor-bias-offcanvas .offcanvas placement="end" trigger-position="none"}
# 🧾 Methods details

### Is the rigor score systematically biased against certain fields?

We took care not to systematically disadvantage certain fields or research styles. Generally, the rigor score is a relative score, computed as “percentage of maximal points” (POMP) score across all indicators that are applicable. For any indicator, one can choose the option "not applicable" if an indicator *principally* cannot be attained by a research output. The points of such non-applicable indicators are removed from the maximum points and therefore do not lower the computed relative rigor score.
:::
...
`r {
  pub_tab <- applicant$indicators[applicant$indicators$type2 == "Publication", ]
  has_multi <- any(pub_tab$P_MultiStudy %in% c("Yes", TRUE), na.rm = TRUE)
  valid_doi <- pub_tab$doi[!is.na(pub_tab$doi) & pub_tab$doi != ""]
  has_dup_doi <- any(duplicated(valid_doi))

  if (has_multi && has_dup_doi) {
    paste0(
      "The applicant submitted ",
      nrow(pub_tab),
      " studies from ",
      dplyr::n_distinct(valid_doi),
      " unique papers (including multiple studies from multi-study papers). ",
      "The ", nrow(pub_tab), " studies had the following methodological type:"
    )
  } else {
    paste0("The ", nrow(pub_tab), " studies had the following methodological type:")
  }
}`

```{r ROs}
dat_tM <- applicant$indicators %>% select(starts_with("P_TypeMethod"), -contains("_Comment"))
...
colnames(dat_tM_tab) <- c("Type of method", "# studies")
...
#| results: "asis"
  ...
  oalex_team <- applicant$OAlex_papers_unique

  cat(paste0(nrow(oalex_team), " out of ", length(unique(applicant$indicators$doi[applicant$indicators$type2 == "Publication"])), " submitted publications could be automatically retrieved with OpenAlex.\n"))
  
  team_tab <- table(oalex_team$team_category) |> as.data.frame()
  team_tab$perc <- paste0(round(team_tab$Freq*100 / nrow(oalex_team)), "%")
...
get_data_types <- function(selector, headings=c("Data type", "# of studies")) {
...
get_data_types(selector="P_Data_Type_") |> kable(table.attr = "class='table table-sm table-striped small' quarto-disable-processing=true") 

if (any(applicant$rigor_pubs$P_Data_Type_Behavioral, na.rm = TRUE)) {
  cat("\n\n### ⤷ Types of behavioral data\n\n")
  get_data_types("P_Data_TypeBehavioral_") |> kable(table.attr = "class='table table-sm table-striped small' quarto-disable-processing=true")  
}
if (any(applicant$rigor_pubs$P_Data_Type_Physiological, na.rm = TRUE)) {
  cat("\n\n### ⤷ Types of physiological data\n\n")
  get_data_types(selector="P_Data_TypePhysiological_") |> kable(table.attr = "class='table table-sm table-striped small' quarto-disable-processing=true")
}
...
Type of population/sample and cultural background for the `r sum(applicant$rigor_pubs$P_Data_Source_NewOwn & applicant$rigor_pubs$P_Sample != "Other", na.rm=TRUE)` studies with own new data collection ("Other" excluded):
...
# in case the Sample_CrossCultural indicator is missing (this might get deselected frequently)
if (all(is.na(df$Sample_CrossCultural))) {
  df$Sample_CrossCultural <- rep("", nrow(df))
}
...
ggplot(heatmap_data, aes(x = Sample_CrossCultural, y = Sample_Type, fill = n)) +
  geom_tile(height = 1) +
  geom_text(aes(label = n), color = "#FFFFFF", size = 3) +
  scale_fill_gradient(low = "#DDDDDD", high = "darkblue") +
  scale_y_discrete(
    limits = rev(levels(df$Sample_Type)),
    expand = c(0, 0)
  ) +
...
```{r experimental_effort}
#| fig-width: 6
#| fig-height: 2
#| out-width: 100%
#| out-height: auto

# show experimental effort as heatmap
if (!is.null(applicant$rigor_pubs$P_DesignExperimental_Effort)) {
  exp_effort <- factor(
    applicant$rigor_pubs$P_DesignExperimental_Effort,
    levels = c("Low", "Medium", "High")
  )

  if (!all(is.na(exp_effort))) {
    effort_data <- table(exp_effort) |> as.data.frame()

    ggplot(effort_data, aes(x = " ", y = exp_effort, fill = Freq)) +
      geom_tile(height = 1) +
      geom_text(aes(label = Freq), color = "#FFFFFF", size = 3) +
      scale_fill_gradient(low = "#DDDDDD", high = "darkblue") +
      scale_y_discrete(limits = rev(c("Low", "Medium", "High"))) +
      scale_x_discrete(expand = c(0, 0)) +
      labs(x = "", y = "", title = "Experimental Effort") +
      theme_minimal() +
      theme(
        legend.position = "none",
        panel.grid = element_blank()
...
if (any(str_detect(colnames(applicant$rigor_pubs), "P_Sample_Location_"))) {
  Sample_Location <- RESQUER:::sum_MC(pubs = applicant$rigor_pubs, prefix = "P_Sample_Location_")
  
  location_data <- data.frame(
    label = rownames(as.data.frame(Sample_Location)),
    Freq  = as.data.frame(Sample_Location)[, 1]
  )

  ggplot(location_data, aes(x = " ", y = label, fill = Freq)) +
    geom_tile(height = 1) +
    geom_text(aes(label = Freq), color = "#FFFFFF", size = 3) +
    scale_fill_gradient(low = "#DDDDDD", high = "darkblue") +
    scale_y_discrete(limits = rev(location_data$label)) +
    scale_x_discrete(expand = c(0, 0)) +
    labs(x = "", y = "", title = "Location of Data Collection") +
    theme_minimal() +
    theme(
      legend.position = "none",
      panel.grid = element_blank()
...
if (any(!is.na(applicant$rigor_pubs$P_Sample_RareOther))) {
  ...
  rare_sample_positions <- which(!is.na(applicant$rigor_pubs$P_Sample_RareOther))
  for (i in rare_sample_positions) {
    res <- paste0(res, "- ", applicant$rigor_pubs$P_Sample_RareOther[i], "\n")
    if (!is.nulla(applicant$rigor_pubs$P_SampleSize[i])) {
...
ggplot(heatmap_data, aes(x = Longitudinal_Type, y = DesignExperimental_Type, fill = n)) +
  ...
  scale_fill_gradient(low = "#DDDDDD", high = "darkblue") +
  scale_y_discrete(
    limits = rev(levels(df$DesignExperimental_Type)),
    expand = c(0, 0)
  ) +
  scale_x_discrete(expand = c(0, 0)) +
  labs(x = "", y = "") +
...
"))
  ...
  credit_barchart(credit_dat=applicant$credit, ordered=FALSE, compact=TRUE)  |> print()
...
::: {.column-margin}
<a class="methods-pill" href="#rrs-methods-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="rrs-methods-offcanvas"><i class="bi bi-journal-text"></i>Methods: Computation of the Relative Rigor Score</a>
:::

:::: {#rrs-methods-offcanvas .offcanvas placement="end" trigger-position="none"}
# 🧾 Methods details

### Computation of the Relative Rigor Score
...
The following charts are based on `r nrow(applicant$rigor_pubs)` scoreable studies.

**What is a "good" Relative Rigor Score?**

The RESQUE indicators cover current best practices, which often are not yet used broadly. Therefore the scores might look meager, even for very good studies. Tentative norm values for the overall rigor score, based on some first evaluation studies are:
...
::: {.column-margin}
<a class="methods-pill" href="#small-charts-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="small-charts-offcanvas"><i class="bi bi-info-circle"></i>How to read the small charts</a>
:::

:::: {#small-charts-offcanvas .offcanvas placement="end" trigger-position="none"}
# 🧾 Methods details

### How to read the small charts

-   *Quantity of openness*: How *often* did they do it?<br>Each small square represents one publication, where the open practice (e.g., open data) has been performed (), *not* performed (), or was not applicable ().
...
::: {.column-margin}
<a class="methods-pill" href="#radar-chart-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="radar-chart-offcanvas"><i class="bi bi-info-circle"></i>How to read the radar chart</a>
:::

:::: {#radar-chart-offcanvas .offcanvas placement="end" trigger-position="none"}
# 🧾 Methods details
...
::: {.column-margin}
<a class="methods-pill" href="#popularity-metric-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="popularity-metric-offcanvas"><i class="bi bi-journal-text"></i>Methods: Popularity metric & CRediT involvement</a>
:::

:::: {#popularity-metric-offcanvas .offcanvas placement="end" trigger-position="none"}

# 🧾 Methods details

### Computation of the Popularity metric
...
### CRediT Levels of involvement

We categorized studies into levels of involvement, based on the degrees of contributorship:
...
```{r prepare_impact_papers}
BIP_unique <- applicant$BIP %>% 
  distinct(doi, .keep_all = TRUE) %>%
  filter(!is.na(pop_class))

```

From `r nrow(BIP_unique)` papers of `r applicant$meta$FullName` that could be retrieved from the BIP! database, `r nrow(BIP_unique %>% filter(pop_class <= "C4"))` `r ifelse(nrow(BIP_unique %>% filter(pop_class <= "C4")) == 1, "was", "were")` in the top 10% popularity class or better.

```{r impact_table}
#| results: asis

# initially, keep all publications (also those not found by BIP).
...
# gather optional free-text comment fields
comment_cols <- names(applicant$indicators)[str_detect(names(applicant$indicators), "_Comment$")]

pop_indicator_cols <- c(
  "doi",
  "title_links_html",
  "P_MultiStudy_Selected",
  "RRS_overall",
  "CRediT_involvement",
...
# Keep only unique DOIs for the impact table (one row per paper, not per study)
# For papers with multiple studies:
#  - Paper metrics (impact, FNCS) are deduplicated per DOI
#  - RRS and CRediT fields are shown per study inside the paper-level cell
pop_sel <- pop_sel |>
  distinct(doi, .keep_all = TRUE) |>
  left_join(
    applicant$indicators |>
      select(all_of(pop_indicator_cols)) |>
      rename(Title = title_links_html) |>
      group_by(doi) |>
      summarise(
        Title = first(Title),
        RRS_overall = {
          labels_raw <- as.character(P_MultiStudy_Selected)
          labels_raw[is.na(labels_raw) | trimws(labels_raw) == ""] <- ""
          n_studies <- dplyr::n()
          labels <- ifelse(
            labels_raw != "",
            labels_raw,
            paste0("Study ", seq_len(n_studies))
          )
          labels <- htmltools::htmlEscape(labels, attribute = FALSE)

          vals <- RRS_overall
          vals_fmt <- ifelse(is.na(vals), "-", paste0(round(vals * 100), "%"))

          if (n_studies > 1) {
            paste0("<b>", labels, ":</b> ", vals_fmt, collapse = "<br>")
          } else {
            vals_fmt[1]
          }
        },
        CRediT_involvement = {
          ...
          vals <- as.character(CRediT_involvement)
          vals[is.na(vals) | trimws(vals) == ""] <- "(n.a.)"
          vals <- htmltools::htmlEscape(vals, attribute = FALSE)

          if (n_studies > 1) {
            paste0("<b>", labels, ":</b> ", vals, collapse = "<br>")
          } else {
            vals[1]
          }
        },
        CRediT_involvement_roles = {
          ...
          vals <- as.character(CRediT_involvement_roles)
          vals[is.na(vals) | trimws(vals) == ""] <- "(n.a.)"
          vals <- str_replace_all(vals, "\\n", "; ")

          # Preserve role markup and render it as italic instead of bold
          vals <- str_replace_all(vals, fixed("<b>"), "<i>")
          vals <- str_replace_all(vals, fixed("</b>"), "</i>")
        ...
        top3_selected = any(!is.na(P_TopPaper_Select) & P_TopPaper_Select %in% c(TRUE, "TRUE", "Yes", "yes", 1)),
        top3_labels = {
          labels_raw <- as.character(P_MultiStudy_Selected)
          labels_raw[is.na(labels_raw) | trimws(labels_raw) == ""] <- ""
          n_studies <- dplyr::n()
        ...
        P_TopPaper_Select = {
          sel <- !is.na(P_TopPaper_Select) & P_TopPaper_Select %in% c(TRUE, "TRUE", "Yes", "yes", 1)
          stars <- ifelse(sel, "⭐", "")
          paste(stars, collapse = "|||")
        },
        P_MeritStatement = {
          ms <- as.character(P_MeritStatement)
          has_ms <- !is.na(ms) & trimws(ms) != ""

          escaped_ms <- htmltools::htmlEscape(ms, attribute = TRUE)
          escaped_ms <- str_replace_all(escaped_ms, "\\n", "<br>")
          escaped_ms <- paste0("<b>Merit Statement</b>:<br>", escaped_ms)

          icons <- ifelse(
            has_ms,
            paste0(
              "<span data-toggle='popover' data-trigger='hover' data-placement='right' data-content=\"",
              escaped_ms,
              "\"><i class='bi bi-info-circle text-primary fs-5'></i></span>"
            ),
            ""
          )
          paste(icons, collapse = "|||")
        },
        across(all_of(comment_cols), first),
        .groups = "drop"
      ),
    by = "doi"
  ) |>
...
if (!("P_TopPaper_Select" %in% names(pop_sel))) {
  pop_sel$P_TopPaper_Select <- ""
}
if (!("P_MeritStatement" %in% names(pop_sel))) {
  pop_sel$P_MeritStatement <- ""
}
if (!("top3_labels" %in% names(pop_sel))) {
  pop_sel$top3_labels <- ""
}
if (!("top3_selected" %in% names(pop_sel))) {
  pop_sel$top3_selected <- FALSE
}

pop_sel$P_TopPaper_Select[is.na(pop_sel$P_TopPaper_Select)] <- ""
pop_sel$P_MeritStatement[is.na(pop_sel$P_MeritStatement)] <- ""
pop_sel$top3_labels[is.na(pop_sel$top3_labels)] <- ""
pop_sel$top3_selected[is.na(pop_sel$top3_selected)] <- FALSE

pop_sel$top3 <- mapply(
  function(label_str, star_str, merit_str) {
    labels <- strsplit(as.character(label_str), "|||", fixed = TRUE)[[1]]
    stars <- strsplit(as.character(star_str), "|||", fixed = TRUE)[[1]]
    merits <- strsplit(as.character(merit_str), "|||", fixed = TRUE)[[1]]

    n_items <- max(length(labels), length(stars), length(merits))
    if (n_items == 0) {
      return("")
    }

    if (length(labels) < n_items) labels <- c(labels, rep("", n_items - length(labels)))
    if (length(stars) < n_items) stars <- c(stars, rep("", n_items - length(stars)))
    if (length(merits) < n_items) merits <- c(merits, rep("", n_items - length(merits)))

    blocks <- character(0)
    for (i in seq_len(n_items)) {
      content_lines <- c(stars[i], merits[i])
      content_lines <- content_lines[trimws(content_lines) != ""]
      ...
      if (n_items > 1) {
        ...
        if (trimws(study_label) == "") {
          study_label <- htmltools::htmlEscape(paste0("Study ", i), attribute = FALSE)
        }
        blocks <- c(
          blocks,
          paste0("<b>", study_label, ":</b><br>", paste(content_lines, collapse = "<br>"))
        )
      } else {
        blocks <- c(blocks, paste(content_lines, collapse = "<br>"))
      }
    }
...
pop_sel$P_TopPaper_Select <- NULL
pop_sel$top3_labels <- NULL
pop_sel <- pop_sel |> relocate(top3, .before = 1)

if (length(comment_cols) > 0) {
  pop_sel$all_comments <- apply(pop_sel[, comment_cols, drop = FALSE], 1, function(row_vals) {
    row_vals <- as.character(row_vals)
    has_text <- !is.na(row_vals) & trimws(row_vals) != ""

    if (!any(has_text)) {
      return(NA_character_)
    }

    escaped_vals <- htmltools::htmlEscape(row_vals[has_text], attribute = TRUE)
    escaped_vals <- str_replace_all(escaped_vals, "\\n", "<br>")

    escaped_names <- htmltools::htmlEscape(comment_cols[has_text], attribute = FALSE)

    paste0("<i>", escaped_names, "</i>: ", escaped_vals, collapse = "<br>")
  })

  if (any(!is.na(pop_sel$all_comments))) {
    pop_sel$comments <- cell_spec(
      ifelse(!is.na(pop_sel$all_comments),
        '<i class="bi bi-info-circle text-primary fs-5"></i>',
        "(n.a.)"
      ),
      escape = FALSE,
      popover = spec_popover(content = pop_sel$all_comments, title = NULL, position = "right")
    )

    # hack: remove popovers for n.a. cells
    pop_sel$comments[is.na(pop_sel$all_comments)] <- cell_spec("(n.a.)",
      escape = FALSE, popover = NULL
    )
  }

  pop_sel[, comment_cols] <- NULL
  pop_sel$all_comments <- NULL
}

pop_sel$P_MeritStatement <- NULL

names(pop_sel)[names(pop_sel) == "three_year_cc"] <- "3 year citation count"
names(pop_sel)[names(pop_sel) == "cc"] <- "Overall citation count"
names(pop_sel)[names(pop_sel) == "RRS_overall"] <- "Rigor Score"
names(pop_sel)[names(pop_sel) == "CRediT_involvement"] <- "Candidates' CRediT involvement"
names(pop_sel)[names(pop_sel) == "CRediT_involvement_roles"] <- "Candidates' CRediT main roles"
names(pop_sel)[names(pop_sel) == "comments"] <- "Applicant's Comments"
names(pop_sel)[names(pop_sel) == "top3"] <- "Top 3 & Merit Statement"
...
if (hideCRediT == TRUE) {
  pop_sel <- pop_sel |> select(-any_of(c("Candidates' CRediT involvement", "Candidates' CRediT main roles")))
}

# First table: Only show high impact publications
high_impact <- pop_sel[which(pop_sel$Popularity %in% c("Top 0.01%", "Top 0.1%", "Top 1%", "Top 10%")), ]

# Second table: Show low-impact publications, or those without any impact score.
...
if (nrow(high_impact) > 0) {
  top3_rows_high <- integer(0)
  if ("top3_selected" %in% names(high_impact)) {
    top3_rows_high <- which(high_impact$top3_selected)
  }
  high_impact_show <- high_impact |> select(-any_of("top3_selected"))

  tab_high <- kable(high_impact_show, row.names = FALSE, escape = FALSE, table.attr = "class='table table-sm small' quarto-disable-processing=true")

  if (length(top3_rows_high) > 0) {
    tab_high <- tab_high |> row_spec(top3_rows_high, background = "#fffce2")
  }
...
if (nrow(low_impact) > 0) {
  top3_rows_low <- integer(0)
  if ("top3_selected" %in% names(low_impact)) {
    top3_rows_low <- which(low_impact$top3_selected)
  }
  low_impact_show <- low_impact |> select(-any_of("top3_selected"))

  tab_low <- kable(low_impact_show, row.names = FALSE, escape = FALSE, table.attr = "class='table table-sm small' quarto-disable-processing=true")

  if (length(top3_rows_low) > 0) {
    tab_low <- tab_low |> row_spec(top3_rows_low, background = "#fffce2")
  }
...
::: {.column-margin}
<a class="warning-pill" href="#not-applicable-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="not-applicable-offcanvas"><i class="bi bi-exclamation-triangle"></i>Why committees should check the "not applicable" justifications</a>
:::

:::: {#not-applicable-offcanvas .offcanvas placement="end" trigger-position="none"}
# ⚠️ Important note for committees

### Gaming RESQUE: Why committees should check the "not applicable" justifications

Choosing "not applicable" indicates that an indicator *principally* cannot be attained by a research output. To avoid bias against certain research fields, the points of such non-applicable indicators are removed from the maximum points and therefore do not lower the computed relative rigor score. 

**However, in order to prevent gaming of this scheme, any "not applicable" claim needs to be justified.** 
Applicants trying to maximize their rigor score will overuse the "not applicable" option, without providing a sound justification.

How to deal with unsound justifications? 

*A (tedious) manual way of adjusting:*
With no or insufficent justification the indicator should be set to "not available" (=0 points) and then the maximum points are not adjusted. This correction currently needs to be done manually in the json file of the applicant. You should also record any changes in a changelog to make transparent what has been changed from the applicant's own self-report.

*"Intuitive" adjustment:*
Previous committees who noticed an excessive usage of the "not applicable" option factored that holistically into the assessment of the candidate, and used that information to ask specific questions in the interview stage.
:::
...
if (any(applicant$preprocessing_notes != "")) {
  cat(paste0(
    "<a class='appendix-pill' href='#preprocessing-notes-offcanvas' data-bs-toggle='offcanvas' role='button' aria-controls='preprocessing-notes-offcanvas'><i class='bi bi-journal-bookmark'></i>Appendix: Preprocessing notes</a>\n\n",
    ":::: {#preprocessing-notes-offcanvas .offcanvas placement='bottom' trigger-position='none'}\n",
    "# Preprocessing Notes\n",
...
<!-- Retrieve later, e.g. with JS:
  const rawJsonBase64 = document.getElementById('raw-json-data').textContent;
  const jsonData = JSON.parse(decodeURIComponent(escape(window.atob(rawJsonBase64))));
  // or, equivalently, use the helper below:
  // const jsonData = getEmbeddedRawJson();
-->
```

```{r embed_json}
#| output: asis
json_content <- paste(applicant_raw_json, collapse = "\n")
json_content_b64 <- jsonlite::base64_enc(charToRaw(json_content))
cat(sprintf('<script id="raw-json-data" type="text/plain" style="display:none;">%s</script>',
            json_content_b64))
...
```{=html}
<script>
function getEmbeddedRawJson() {
  const el = document.getElementById('raw-json-data');
  if (!el) return null;
  return JSON.parse(decodeURIComponent(escape(window.atob(el.textContent))));
}
</script>
```
