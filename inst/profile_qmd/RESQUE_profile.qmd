...
params:
  ...
  json_path: '/Users/felix/LMU/DGPs Kommission Open Science/RESQUE/resque_schönbrodt_2026-03.json'
  show_inter: true
---
...
if ("P_TopPaper_Select" %in% names(pop_sel)) {
  pop_sel$top3_selected <- !is.na(pop_sel$P_TopPaper_Select) & pop_sel$P_TopPaper_Select %in% c(TRUE, "TRUE", "Yes", "yes", 1)
  pop_sel$top3 <- ifelse(
    !is.na(pop_sel$P_TopPaper_Select) & pop_sel$P_TopPaper_Select %in% c(TRUE, "TRUE", "Yes", "yes", 1),
    "⭐",
    ""
  )
...
merit_text <- as.character(pop_sel$P_MeritStatement)
has_merit <- !is.na(merit_text) & trimws(merit_text) != ""

if (any(has_merit)) {
  escaped_merit <- htmltools::htmlEscape(merit_text, attribute = TRUE)
  escaped_merit <- str_replace_all(escaped_merit, "\\n", "<br>")
  escaped_merit <- paste0("<b>Merit Statement</b>:<br>", escaped_merit)

  merit_icon <- cell_spec(
    ifelse(has_merit,
      '<i class="bi bi-info-circle text-primary fs-5"></i>',
      ""
    ),
    escape = FALSE,
    popover = spec_popover(content = ifelse(has_merit, escaped_merit, NA_character_), title = NULL, position = "right")
  )

  top3_star <- ifelse(pop_sel$top3_selected, "⭐", "")
  pop_sel$top3 <- ifelse(
    top3_star != "" & has_merit,
    paste0(top3_star, "<br>", merit_icon),
    ifelse(top3_star != "", top3_star, ifelse(has_merit, merit_icon, ""))
  )
}

# Merit statement is represented in Top 3 as an icon popover; remove raw column
