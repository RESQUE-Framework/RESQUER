...
format:
  html:
    ...
    toc-location: left
    df-print: paged
    css: [assets/overview_styles.css, assets/profile_styles.css]
    page-layout: full
    grid:
      sidebar-width: 250px
      body-width: 900px
      ...
      gutter-width: 1.5rem
params:
  json_folder: '/Users/felix/Documents/Github/RESQUE-Framework/RESQUER/_test/report_testsuite/jsons'
  anonymous: true
  clear_cache: false
  verbose: false
  randomize: false
...
# params=list(); params$verbose=FALSE; params$anonymous=FALSE; params$randomize = FALSE; params$clear_cache=TRUE; params$json_folder='/Users/felix/Documents/Github/RESQUE-Framework/RESQUER/_test/report_testsuite/jsons'; params$FNCS_type = "mean"
...
if (is.null(params$json_folder)) {
  ...
  if (file.exists(cache_file)) {
    ...
    for (i in 1:length(json_files)) {
        if (params$verbose == TRUE) print(paste0("Reading file #", i, ": ", json_files[i]))
        R <- read_RESQUE(file=json_files[i], verbose=FALSE)
        applicant_list[[i]] <- preprocess(applicant=R, verbose=FALSE)
        
        # OpenAlex API rate limit max 10 requests every second - wait a little bit.
        Sys.sleep(0.5)
    }
...
# for demo purposes: Wiggle some numbers to make them anonymous
  if (params$randomize == TRUE) {
    for (i in 1:length(applicant_list)) {
      applicant_list[[i]]$RRS$overall_score <- wiggle(applicant_list[[i]]$RRS$overall_score)
    }
  }
...
for (i in 1:length(applicant_list)) {
  row <- get_RRS_tablerow(applicant=applicant_list[[i]], add_scores = TRUE)
  
  if (i > 1 && any(colnames(tab_RRS) != colnames(row))) {
    warning("Colnames do not match - the json files do not have the same set of indicators!")
    colnames(row) <- colnames(tab_RRS)
  } 
  
  tab_RRS <- rbind(tab_RRS, row)  
}
...
datatable(
  ...
  options = list(
    ...
    paging = FALSE,
    columnDefs = list(
      list(targets = colnames(tab_RRS)[2], orderData = 7), 
      list(targets = colnames(tab_RRS)[3], orderData = 8),  # Sort OD by RRS_OD (which is the 5th column)
      list(targets = colnames(tab_RRS)[4], orderData = 9),
      list(targets = colnames(tab_RRS)[5], orderData = 11),
      list(targets = colnames(tab_RRS)[6], orderData = 10),  
      
      list (targets = c(7:11), visible = FALSE) # Hide numeric sorting columns
    ))
)
...
for (i in 1:nrow(tab_impact)) {
  tab_impact$BIP_n_papers_top10[i] <- waffle_html(
    ...
    max_value = sum(top10[i], non_top10[i]), rows = 1,
...
if (params$FNCS_type == "mean") {
  tab_impact$avg_FNCS = sapply(applicant_list, function(x) mean(x$FNCS$FNCS, na.rm=TRUE) |> round(1))
  FNCS_colname <- "Mean FNCS"
}

if (params$FNCS_type == "median") {
  tab_impact$avg_FNCS = sapply(applicant_list, function(x) median(x$FNCS$FNCS, na.rm=TRUE) |> round(1))
  FNCS_colname <- "Median FNCS"
}

FNCS_quantiles <- quantile(tab_impact$avg_FNCS, probs = c(0.25, 0.75))

if (params$randomize == TRUE) {
  tab_impact$avg_FNCS <- wiggle(tab_impact$avg_FNCS)
}

tab_impact$highest_citation_count = 
  sapply(applicant_list, function(x) {
    top <- which.max(x$FNCS$cited_by_count)
    max_citation <- x$FNCS$cited_by_count[top]
    year_of_max_citation <- x$FNCS$publication_year[top]

    if (params$randomize == TRUE) {
      max_citation <- wiggle(max_citation)
      if (max_citation < 1) max_citation <- 1
      year_of_max_citation <- year_of_max_citation - sample(1:7, 1)
    }

    paste0(
      max_citation, " (",
      year_of_max_citation,
      ")"
    )
  })
...
datatable(
  ...
  options = list(
    ...
    dom = 't', 
    paging = FALSE),
  colnames=c("Applicant", "# of papers in top-10% of popularity", FNCS_colname, "Highest citation count of a single paper")) %>%
  formatStyle(
    'avg_FNCS',
    backgroundColor = styleInterval(
      FNCS_quantiles, c('#fbe3cc', '#fbf9cc', '#daf9d7')
    )
  )
```
...
::: {.column-margin}
<a class="methods-pill" href="#impact-indices-offcanvas" data-bs-toggle="offcanvas" role="button" aria-controls="impact-indices-offcanvas"><i class="bi bi-info-circle"></i>How to interpret the indices</a>
:::

:::: {#impact-indices-offcanvas .offcanvas placement="end" trigger-position="none"}
# 🧾 Methods details
...
tab_combi$age <- sapply(applicant_list, function(x) x$meta$AcademicAge)

tab_combi$name <- paste0(tab_combi$applicant, " (age: ", tab_combi$age, ")")

ggplot(tab_combi, aes(x=RRS_overall, y=avg_FNCS)) + 
  geom_point() + 
  geom_text_repel(
    aes(label=name),
    hjust=0,
  ...
  theme_bw() +
  xlab("Overall Rigor Score (higher ist better)") + ylab(paste0(FNCS_colname, " (higher ist better)")) +
...
<a class='appendix-pill' href='#session-info-offcanvas' data-bs-toggle='offcanvas' role='button' aria-controls='session-info-offcanvas'><i class='bi bi-journal-bookmark'></i>Appendix: Session info</a>

:::: {#session-info-offcanvas .offcanvas placement='bottom' trigger-position='none'}
# Session info
