...
format:
  html:
    ...
    toc-location: left
    df-print: paged
    css: [assets/overview_styles.css, profile_styles.css]
    page-layout: full
    grid:
      sidebar-width: 250px
      body-width: 900px
...
<!-- Params:
randomize: For demo purposes, randomize some values to change them away from the real values.
...
if (is.null(params$json_folder)) {
  ...
  if (params$randomize == TRUE) {
    for (i in 1:length(applicant_list)) {
      ...
      applicant_list[[i]]$RRS$sector_scores$rel_score <- sapply(applicant_list[[i]]$RRS$sector_scores$rel_score, wiggle)
    }
  }
}
...
*(Note: The Overall Rigor Score presented here is based solely on the four displayed transparency categories, excluding, for example, *Methodological Rigor*. As a result, it can slightly differ from the Overall Score in individual reports.)*
...
for (i in 1:length(applicant_list)) {
  ...
  if (i > 1 && any(colnames(tab_RRS) != colnames(row))) {
    warning("Colnames do not match - the json files donot have the same set of indicators!")
